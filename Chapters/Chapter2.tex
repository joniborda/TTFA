...
En esta sección se presentan algunos requerimientos establecidos para el trabajo. En cada requerimiento se detalla su objetivo y alcance, con el fin de precisar las condiciones necesarias para el desarrollo y la correcta implementación del modelo propuesto.
	...
	\begin{itemize}
		...
		\item Descarga de predicciones de ventas en formato \texttt{CSV}: el sistema debe permitir la exportación de las predicciones de ventas a archivos \texttt{CSV} para su análisis externo o integración con otras herramientas de planificación.
	\end{itemize}
...
A continuación, se describen los tipos de modelos aplicados:
	...
	\item SARIMA (\textit{Seasonal Autoregressive Integrated Moving Average}) \citep{boxjenkins1970}: se aplicó para modelar la estacionalidad y tendencia presentes en los datos de ventas. SARIMA es especialmente efectivo cuando el comportamiento temporal es predominante y estable a lo largo del tiempo.
	...
	\item \textit{Temporal Fusion Transformer} (TFT) \citep{lim2021temporal}: se implementó este modelo de vanguardia por su capacidad de atención interpretable, que permite identificar qué variables exógenas y qué períodos históricos influyen en cada predicción. El TFT resulta particularmente adecuado para el contexto de Latech, donde es crucial entender el impacto de las inversiones publicitarias en diferentes horizontes temporales.
...
Las características más relevantes para este trabajo son:
\begin{itemize}
...
\begin{itemize}
	\item MLflow \citep{mlflow}: se utiliza como herramienta principal para estructurar y ejecutar pipelines de datos y experimentos, con registro automático de artefactos y trazabilidad.
...
Cabe destacar que la ejecución técnica de estos flujos y el entrenamiento de los modelos se realizaron íntegramente en una configuración local. El uso de Docker garantiza la consistencia y portabilidad de este entorno, lo que facilita la transición hacia la etapa productiva. Una vez finalizado el trabajo, el sistema se desplegará en los servidores del cliente, donde se asegurará que el entorno cumpla con los protocolos de seguridad y disponibilidad requeridos para su operación definitiva.