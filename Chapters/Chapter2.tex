...
A continuación, se describen los tipos de modelos aplicados:
	...
	\item GRU (\textit{Gated Recurrent Unit}) \citep{cho2014gru}: se evaluó como una variante más simple y computacionalmente eficiente de las LSTM, útil cuando los recursos de entrenamiento son limitados.
...
\subsection{Exposición de servicios: FastAPI \citep{fastapi}}
...
\textbf{Docker} \citep{docker} permite definir entornos de ejecución consistentes para los servicios del proyecto. Sus contenedores aseguran reproducibilidad, aislamiento y portabilidad entre distintos entornos.

\subsection{Orquestación y pipelines: Metaflow y Airflow}
\textbf{Metaflow} \citep{metaflow} se utiliza como herramienta principal para estructurar y ejecutar pipelines de datos y experimentos, con registro automático de artefactos y trazabilidad.

\textbf{Airflow} \citep{airflow} se considera una alternativa válida para tareas recurrentes o programaciones complejas, pero no se adopta en esta etapa del proyecto debido a su mayor complejidad operativa.
