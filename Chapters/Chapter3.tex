...
\begin{itemize}
    ...
    \begin{itemize}
	    ...
	    \item \texttt{created\_month}: representa el mes calendario del 1 al 12, donde 1 representa enero y 12 diciembre. Mediante este indicador, los algoritmos analizan la estacionalidad anual y el impacto de periodos de alta demanda, como las festividades o promociones de largo plazo. Esta característica facilita que el modelo \textit{Temporal Fusion Transformer} correlacionen meses específicos con fluctuaciones históricas, lo cual mejora la precisión de los pronósticos en horizontes temporales extendidos.
...
\item Transformación de ventanas deslizantes (\textit{sliding window}): se aplicó la función \texttt{shift()} para la generación de variables de retraso (conocidas en inglés como \textit{lags}) en el conjunto de datos. A través de este método, el sistema crea pares de características históricas ($x$) y valores objetivo ($y$) como:
...
Esta fase del proyecto consistió en la implementación técnica y el ajuste de las arquitecturas encargadas de transformar el conjunto de datos consolidado en proyecciones de demanda. El desarrollo se estructuró a partir de un modelo base heurístico que funcionó como punto de referencia (en inglés \textit{baseline}) frente a la aplicación de diversos algoritmos de aprendizaje automático de mayor complejidad.
...
\begin{itemize}
	...
	\item Promedio móvil (\textit{en inglés trailing mean}): calcula la media de ventas de los últimos siete días previos a la fecha objetivo. 
