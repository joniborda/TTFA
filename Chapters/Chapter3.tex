...
\begin{itemize}
	\item created: que registra la fecha de creación de cada orden.
	\item totalPrice: correspondiente al monto total pagado.
	\item customerId: que identifica al cliente.
	\item lineItems: donde se detalla cada producto incluido en la compra junto con sus cantidades.
	\item channel: que indica el canal por el cual ingresó la orden.
	\item tags: que permite distinguir si la compra corresponde a un cliente nuevo o recurrente mediante etiquetas provistas por Shopify.
	\item orderNumberForCustomer: que señala el número de orden que representa dentro del historial del cliente (por ejemplo, un valor igual a 1 implica un cliente nuevo).
	\item diffWeeksFromFirstPurchase: que expresa la cantidad de semanas transcurridas desde la primera compra del usuario, permitiendo analizar patrones de frecuencia y retención.
...
\begin{itemize}
	\item created\_weekday: que identifica el día de la semana de cada orden.
	\item created\_month: que permite estudiar estacionalidades mensuales. 
	\item unique\_customers: que calcula la cantidad de clientes distintos por día.
	\item new\_customers: que contabiliza cuántos usuarios realizaron su primera compra dentro de cada fecha.
	\item returning\_customers: que contabiliza cuántos usuarios realizaron más de una compra. 
\end{itemize}
...
Esta capa es responsable del entrenamiento, evaluación y selección de los modelos de pronóstico de ventas. Se implementa un enfoque comparativo donde se ejecutan simultáneamente los modelos mencionados en el Capítulo 2.2 (SARIMA, \textit{Exponential Smoothing}, \textit{Random Forest}, XGBoost, LightGBM, LSTM y \textit{Prophet}), con el objetivo de identificar el que ofrezca el mejor desempeño predictivo según las métricas definidas.

El proceso de modelado sigue las siguientes etapas:

Entrenamiento múltiple: todos los modelos son entrenados utilizando el mismo conjunto de datos históricos consolidado y enriquecido.

Evaluación y métricas: cada modelo es evaluado con la métrica RMSE (Raíz del Error Cuadrático Medio)  para penalizar errores grandes que impactan significativamente en la planificación financiera y de producción.

Selección automática: el modelo que obtiene el menor valor de RMSE en el conjunto de validación es seleccionado automáticamente para su uso en producción.

Persistencia del modelo: una vez seleccionado, el modelo ganador es serializado y almacenado en un registro de modelos donde queda disponible para su uso en las predicciones futuras.

Exposición mediante API: el modelo guardado es integrado en un endpoint REST que forma parte del sistema de microservicios para que el sistema Inventory Tracker consulte las predicciones de ventas en tiempo real.
