...
\begin{itemize}
    ...
    \begin{itemize}
	    \item \textit{created\_weekday}: representa el día de la semana con un valor numérico entre el 0 y el 6, donde 0 equivale el día lunes y el 6 el domingo. Su uso es fundamental para que modelos como Random Forest o XGBoost identifiquen patrones de comportamiento diferenciados entre los días laborables y el fin de semana, periodos donde la actividad comercial de Latech suele presentar variaciones significativas.
	    \item \textit{created\_month}: representa el mes calendario del 1 al 12, donde 1 representa enero y 12 diciembre. Mediante este indicador, los algoritmos analizan la estacionalidad anual y el impacto de periodos de alta demanda, como las festividades o promociones de largo plazo. Esta característica facilita que el modelo \textit{Temporal Fusion Transformer} correlacionen meses específicos con fluctuaciones históricas, lo cual mejora la precisión de los pronósticos en horizontes temporales extendidos.
...
\item Transformación de ventanas deslizantes (en inglés \textit{sliding window}): se aplicó la función \texttt{shift()} para la generación de variables de retraso (conocidas en inglés como \textit{lags}) en el conjunto de datos. A través de este método, el sistema crea pares de características históricas ($x$) y valores objetivo ($y$) como:
	...
	\begin{itemize}
	    \item adSpend \_lag\_7: contiene el gasto en publicidad desplazado 7 días hacia atrás.
	    \item adSpend\_lag\_30: contiene el gasto en publicidad desplazado 30 días hacia atrás.
	    \item orders\_lag\_7: contiene la cantidad de pedidos 7 días hacia atrás.
	    \item orders\_lag\_30: contiene la cantidad de pedidos 30 días hacia atrás.
	...
	\item Métricas de comportamiento del cliente: se derivaron indicadores de fidelidad a partir de la actividad transaccional diaria. El sistema identifica el volumen de nuevos compradores mediante el filtrado de registros donde el número de orden del cliente (\textit{order\_number\_for\_customer}) es igual a 1. Esta lógica permite distinguir la primera compra de cada usuario de las adquisiciones recurrentes. Además, se calculó la cantidad de clientes únicos diarios y la antigüedad del usuario en semanas desde su primera interacción reprsentada con la variable \textit{diff\_weeks\_from\_first\_purchase}, lo que aporta información sobre la retención y el ciclo de vida del cliente.
...
\begin{itemize}
	...
	\item Servicio de ingestión: coordina los procesos de cron y los conectores para las APIs externas de Shopify y Triple Whale.
...
La base de datos PostgreSQL funcionó como el núcleo de almacenamiento estructurado para los datos consolidados. Esta integración permitió que el sistema mantuviera su autonomía ante interrupciones en las APIs externas y optimizó la velocidad de consulta durante el entrenamiento de los modelos
. Los microservicios interactuaron con las tablas orders y forecast de manera acotada, lo cual protegió la integridad del esquema de datos diseñado.
...
\begin{itemize}
	...
	\item Persistencia de Datos Enriquecidos: La base de datos no solo almacenó registros crudos, sino que albergó información procesada y enriquecida con variables críticas para el negocio, tales como la distinción entre clientes nuevos y recurrentes, el cálculo de semanas desde la primera compra (diffWeeksFromFirstPurchase) y la identificación de días de la semana o meses para capturar la estacionalidad.
	\item Gestión de Artefactos y Modelos (AWS S3): Complementando a PostgreSQL, se integró AWS S3 para el almacenamiento de objetos de gran tamaño, tales como los modelos serializados, archivos de configuración y datos intermedios en formato Parquet. Esta infraestructura aseguró la durabilidad y escalabilidad necesaria para manejar el historial creciente de experimentos y versiones de modelos.
	...
	\item Documentación Técnica Automatizada: La integración con FastAPI generó automáticamente documentación interactiva bajo el estándar OpenAPI (Swagger)
. Esta herramienta facilitó las pruebas de integración para los desarrolladores de Latech, permitiendo probar los endpoints de predicción y refresco de datos de forma aislada y documentada.
	\item Seguridad y Trazabilidad: La capa de persistencia incluyó mecanismos de auditoría mediante logs de ejecución, registrando el volumen de datos procesados, la fecha de la última predicción generada y cualquier incidencia detectada durante la ingesta o el entrenamiento. Además, se contempló la anonimización de datos sensibles para cumplir con la Ley de Protección de Datos Personales.
